...
class RateLimitTester:
    ...
    def __init__(self, base_url: str = "http://127.0.0.1:8000", max_concurrency: int = 10):
        """
    ...
    async def _make_request(self, endpoint: str, headers: Optional[Dict[str, str]] = None) -> httpx.Response:
        """Make a single HTTP request to an endpoint."""
        url = f"{self.base_url}{endpoint}"
        try:
            return await self._client.get(url, headers=headers or {})
        except Exception as exc:
            print(f"  Error making request to {endpoint}: {exc}")
            raise
    ...
    async def test_endpoint(
        ...
        lock = asyncio.Lock()
        concurrency = min(max_concurrency or self.max_concurrency, max(1, requests_to_make))
        semaphore = asyncio.Semaphore(concurrency)

        async def send(i: int) -> None:
            nonlocal successful_requests, rate_limited_requests, last_headers
            async with semaphore:
                try:
                    response = await self._make_request(endpoint, headers)
                    response_headers = self._extract_rate_limit_headers(response)
                    status_code = response.status_code
                    if status_code == 200:
                        message = f"   [{i+1}/{requests_to_make}] ✅ Status: {status_code}"
                    elif status_code == 429:
                        message = f"   [{i+1}/{requests_to_make}] 🚫 Rate limited: {status_code}"
                    else:
                        message = f"   [{i+1}/{requests_to_make}] ⚠️  Unexpected status: {status_code}"
                    ...
                    async with lock:
                        ...
                        print(f"   [{i+1}/{requests_to_make}] ❌ Error: {exc}")
                finally:
                    if delay_between_requests and concurrency == 1 and i < requests_to_make - 1:
                        await asyncio.sleep(delay_between_requests)

        await asyncio.gather(*(send(i) for i in range(requests_to_make)))
    ...
    async def test_different_ips(self, endpoint: str, requests_per_ip: int = 20):
